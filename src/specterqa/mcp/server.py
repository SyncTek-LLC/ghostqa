"""SpecterQA MCP Server — Model Context Protocol server for AI agent integration.

Exposes SpecterQA's behavioral testing engine as MCP tools so that AI agents
can discover products, run tests, and retrieve results programmatically.

Usage:
    specterqa-mcp            # stdio transport (default)
    python -m specterqa.mcp  # alternative invocation

The server provides four tools:

    specterqa_run            Run behavioral tests against a product
    specterqa_list_products  List available products and their journeys
    specterqa_get_results    Retrieve results from a previous run
...
def _resolve_project_dir(directory: str | None = None) -> Path:
    ...
    Returns the path to the .specterqa directory, or a default if none found.
    ...
    # Fallback: use start/.specterqa (may not exist yet)
...
def _check_project_initialized(project_dir: Path) -> str | None:
    ...
    if not project_dir.is_dir():
        return (
            ...
            "Run specterqa_init first, or use the specterqa CLI:\n"
            "  specterqa init --dir /path/to/your/project"
        )
    return None
...
def _check_playwright_available() -> str | None:
    ...
    try:
        ...
        return (
            "Playwright is not installed. SpecterQA requires Playwright for browser-based testing.\n\n"
            "Install it:\n"
...
def _build_config(
    ...
    """Build a SpecterQAConfig from the project directory.

    Returns:
        A SpecterQAConfig instance.
    ...
    try:
        from specterqa.credentials import resolve_api_key
...
def create_server() -> Any:
    """Create and configure the SpecterQA MCP server.
    ...
    try:
        ...
        raise ImportError(
            "The 'mcp' package is required for the SpecterQA MCP server.\n\n"
            "Install it:\n"
    ...
    mcp = FastMCP(
        "specterqa",
        instructions=(
            "SpecterQA is an AI-powered behavioral testing tool. "
    ...
    async def specterqa_run(
        ...
        Args:
            product: Product name (must match a YAML file in .specterqa/products/).
            ...
            directory: Working directory to search for .specterqa/ project.
    ...
    # ── Tool: specterqa_list_products ──────────────────────────────────────

    @mcp.tool(
        name="specterqa_list_products",
        description=(
            "List all available SpecterQA products and their journeys. "
            "Shows product names, base URLs, app types, and which journeys "
            "are configured for testing."
        ),
    )
    async def specterqa_list_products(
        directory: str | None = None,
    ) -> str:
    ...
    # ── Tool: specterqa_get_results ────────────────────────────────────────

    @mcp.tool(
        name="specterqa_get_results",
        description=(
            "Get full results from a previous SpecterQA run by run ID. "
            "Returns the complete structured result including step reports, "
            "findings, cost breakdown, and evidence paths."
        ),
    )
    async def specterqa_get_results(
        run_id: str,
        directory: str | None = None,
    ) -> str:
        """Get results from a previous SpecterQA run.
            ...
            return json.dumps(
                {
                    ...
                    "error_code": "RUN_NOT_FOUND",
                    "available_run_ids": [],
                    "hint": "No runs have been recorded yet. Use specterqa_run to execute tests first.",
                }
            )
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "Creates the .specterqa/ directory with config, sample products, "
            "personas, and journeys. Use this before running any tests."
        ),
    )
    async def specterqa_init(
        directory: str = ".",
        url: str | None = None,
    ) -> str:
        """Initialize a SpecterQA project.

        Args:
            directory: Parent directory for .specterqa/ project. Default: current directory.
        ...
        project_dir = target / ".specterqa"

        if project_dir.exists():
            return json.dumps(
                {
                    "success": False,
                    "error": f"SpecterQA project already initialized at {project_dir}",
                    "hint": "The .specterqa/ directory already exists. Delete it first to re-initialize.",
                }
            )
        ...
        try:
            for sub in subdirs:
                ...
                created_files.append(f".specterqa/{sub}/")
            ...
            created_files.append(".specterqa/config.yaml")
            ...
            created_files.append(".specterqa/products/demo.yaml")
            ...
            created_files.append(".specterqa/personas/alex-developer.yaml")
            ...
            created_files.append(".specterqa/journeys/demo-onboarding.yaml")

            # SECURITY (FIND-003): Ensure .gitignore in the parent directory
            # includes .specterqa/personas/ so persona files (which may contain
            # credential environment variable references) are not committed.
            _personas_entry = ".specterqa/personas/"
            ...
            created_files.append(".gitignore (updated with .specterqa/personas/)")
        ...
        return json.dumps(
            {
                ...
                "next_steps": [
                    "Edit .specterqa/products/demo.yaml with your app's URL" + (f" (set to {url})" if url else ""),
                    ...
                    *([] if api_key_configured else ["Set ANTHROPIC_API_KEY: export ANTHROPIC_API_KEY=sk-ant-..."]),
                    "Run tests: use the specterqa_run tool with product='demo'",
                ],
            }
        )

    # ── Tool: specterqa_budget_check ───────────────────────────────────────

    @mcp.tool(
        name="specterqa_budget_check",
        description=(
            "Check cumulative SpecterQA cost against daily and monthly budget limits. "
            "Call this before specterqa_run to avoid hard budget-exceeded errors mid-run. "
            "Returns current spend, limits, and whether a run can proceed. Zero cost — no API calls made."
        ),
    )
    async def specterqa_budget_check(
        directory: str | None = None,
...
def _find_journeys_for_product(project_dir: Path, product_name: str) -> list[dict[str, str]]:
    ...
    # Check product-scoped journeys (inside .specterqa/<product>/journeys/)
...
def main() -> None:
    """Entry point for the specterqa-mcp command."""
