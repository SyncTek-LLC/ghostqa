"""SpecterQA Dashboard Server — Local HTTP server for browsing test run evidence.

Serves a Jinja2-rendered web dashboard on localhost that lets users browse
SpecterQA run results, step-by-step details, screenshots, and findings.

Usage:
    from specterqa.viewer.server import DashboardServer
    server = DashboardServer(evidence_dir=Path(".specterqa/evidence"), port=8199)
...
logger = logging.getLogger("specterqa.viewer")
...
class _DashboardHandler(BaseHTTPRequestHandler):
    """HTTP request handler for the SpecterQA dashboard."""
...
<head>
    ...
    <title>SpecterQA — {status.value} {status.phrase}</title>
...
<body>
    <header class="site-header">
        <div class="header-content">
            <a href="/" class="logo">
                <span class="logo-icon">&#128123;</span> SpecterQA
            </a>
        </div>
    </header>
    ...
    <footer class="site-footer">
        <div class="container">SpecterQA v0.3.0 &mdash; AI Behavioral Testing</div>
    </footer>
</body>
</html>"""
...
class DashboardServer:
    """Manages a local HTTP server for the SpecterQA dashboard.
