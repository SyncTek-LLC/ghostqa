"""Unit and component tests for specterqa.mcp.server.
...
with the SPECTERQA_ALLOWED_DIRS env var patched to include tmp_path, and
...
Test structure:
    ...
    TestSpecterqaInitTool         — specterqa_init MCP tool
    TestSpecterqaListProductsTool — specterqa_list_products MCP tool
    TestSpecterqaGetResultsTool   — specterqa_get_results MCP tool
    TestSpecterqaRunTool          — specterqa_run MCP tool (error paths + mocked runs)
"""
...
def _make_server_with_allowed(tmp_path: Path):
    ...
    The allowlist (_ALLOWED_DIRS) defaults to None when SPECTERQA_ALLOWED_DIRS
    ...
    """
    import specterqa.mcp.server as srv_module
    from specterqa.mcp.server import create_server
...
def _make_initialized_project(tmp_path: Path) -> Path:
    """Create a minimal .specterqa/ directory with all standard subdirs."""
...
class TestBuildAllowedDirs:
    """_build_allowed_dirs() reads SPECTERQA_ALLOWED_DIRS.
    ...
    def test_parses_single_path_from_env(self, tmp_path: Path):
        ...
        with patch.dict(os.environ, {"SPECTERQA_ALLOWED_DIRS": str(tmp_path)}):
            result = _build_allowed_dirs()
    ...
    def test_parses_colon_separated_paths(self, tmp_path: Path):
        ...
        p2.mkdir()
        env_val = f"{p1}:{p2}"
        with patch.dict(os.environ, {"SPECTERQA_ALLOWED_DIRS": env_val}):
            result = _build_allowed_dirs()
    ...
    def test_ignores_blank_entries_in_colon_list(self, tmp_path: Path):
        ...
        with patch.dict(os.environ, {"SPECTERQA_ALLOWED_DIRS": f"{tmp_path}::"}):
            result = _build_allowed_dirs()
...
class TestValidateDirectory:
    ...
    def test_allows_any_path_when_no_restriction(self, tmp_path: Path):
        ...
        import specterqa.mcp.server as srv_module
        from specterqa.mcp.server import _validate_directory
...
class TestResolveProjectDir:
    """_resolve_project_dir() should locate .specterqa/ by walking up from start."""

    def test_finds_specterqa_in_start_directory(self, tmp_path: Path):
        from specterqa.mcp.server import _resolve_project_dir

        specterqa_dir = tmp_path / ".specterqa"
        specterqa_dir.mkdir()

        result = _resolve_project_dir(str(tmp_path))
        assert result == specterqa_dir

    def test_finds_specterqa_in_parent_directory(self, tmp_path: Path):
        from specterqa.mcp.server import _resolve_project_dir
    ...
    def test_returns_default_when_not_found(self, tmp_path: Path):
        ...
        assert result == isolated / ".specterqa"
    ...
    def test_defaults_to_cwd_when_directory_is_none(self):
        ...
        assert result.name == ".specterqa"

    def test_finds_specterqa_exactly_one_level_up(self, tmp_path: Path):
        from specterqa.mcp.server import _resolve_project_dir
    ...
    def test_prefers_nearer_specterqa_over_distant_ancestor(self, tmp_path: Path):
        from specterqa.mcp.server import _resolve_project_dir

        (tmp_path / ".specterqa").mkdir()
        sub = tmp_path / "sub"
        sub.mkdir()
        sub_specterqa = sub / ".specterqa"
        sub_specterqa.mkdir()

        result = _resolve_project_dir(str(sub))
        assert result == sub_specterqa
...
class TestCheckProjectInitialized:
    ...
    def test_returns_none_for_existing_project_dir(self, tmp_path: Path):
        from specterqa.mcp.server import _check_project_initialized
    ...
    def test_error_message_contains_init_hint(self, tmp_path: Path):
        ...
        project_dir = tmp_path / ".specterqa"
        result = _check_project_initialized(project_dir)
        assert "specterqa_init" in result or "specterqa init" in result

    def test_returns_none_for_nested_project_dir(self, tmp_path: Path):
        from specterqa.mcp.server import _check_project_initialized

        nested = tmp_path / "a" / "b" / ".specterqa"
...
class TestCheckPlaywrightAvailable:
    ...
    def test_returns_none_when_playwright_importable(self):
        from specterqa.mcp.server import _check_playwright_available
...
class TestJsonSerialize:
    ...
    def test_serializes_path_to_string(self, tmp_path: Path):
        from specterqa.mcp.server import _json_serialize
    ...
    def test_serializes_path_preserves_separator(self):
        ...
        p = Path("/usr/local/bin/specterqa")
        assert _json_serialize(p) == "/usr/local/bin/specterqa"
...
class TestListAllRunIds:
    ...
    def test_returns_empty_list_for_nonexistent_dir(self, tmp_path: Path):
        from specterqa.mcp.server import _list_all_run_ids
...
class TestLoadRunResult:
    ...
    def test_returns_none_for_missing_run_id(self, tmp_path: Path):
        from specterqa.mcp.server import _load_run_result
...
class TestFindJourneysForProduct:
    ...
    def test_returns_empty_list_when_no_journeys_dir(self, tmp_path: Path):
        from specterqa.mcp.server import _find_journeys_for_product
...
class TestBuildConfig:
    """_build_config() produces a SpecterQAConfig from the project directory."""

    def test_builds_config_without_yaml_file(self, tmp_path: Path):
        from specterqa.mcp.server import _build_config
...
# ---------------------------------------------------------------------------
# 9. specterqa_init MCP tool
# ---------------------------------------------------------------------------

class TestSpecterqaInitTool:
    """specterqa_init tool creates .specterqa/ directory structure."""

    def test_creates_success_response(self, tmp_path: Path):
        mcp, srv_module, original = _make_server_with_allowed(tmp_path)
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_init", {"directory": str(tmp_path)})
    ...
    def test_creates_specterqa_directory(self, tmp_path: Path):
        ...
        try:
            async def _run():
                await mcp.call_tool("specterqa_init", {"directory": str(tmp_path)})

            run_async(_run())
            assert (tmp_path / ".specterqa").is_dir()
        finally:
            _restore_allowed(srv_module, original)
    ...
    def test_creates_all_subdirectories(self, tmp_path: Path):
        ...
        try:
            ...
            specterqa_dir = tmp_path / ".specterqa"
            for sub in ("products", "personas", "journeys", "evidence"):
                assert (specterqa_dir / sub).is_dir(), f"Missing subdirectory: {sub}/"
        finally:
            _restore_allowed(srv_module, original)
    ...
    def test_creates_sample_product_yaml(self, tmp_path: Path):
        ...
        try:
            ...
            run_async(_run())
            assert (tmp_path / ".specterqa" / "products" / "demo.yaml").is_file()
        finally:
            _restore_allowed(srv_module, original)
    ...
    def test_creates_sample_persona_yaml(self, tmp_path: Path):
        ...
        try:
            ...
            run_async(_run())
            assert (tmp_path / ".specterqa" / "personas" / "alex-developer.yaml").is_file()
        finally:
            _restore_allowed(srv_module, original)
    ...
    def test_creates_sample_journey_yaml(self, tmp_path: Path):
        ...
        try:
            ...
            run_async(_run())
            assert (tmp_path / ".specterqa" / "journeys" / "demo-onboarding.yaml").is_file()
        finally:
            _restore_allowed(srv_module, original)
    ...
    def test_creates_gitignore(self, tmp_path: Path):
        ...
        try:
            ...
            assert ".specterqa/personas/" in content
        finally:
            _restore_allowed(srv_module, original)
    ...
    def test_url_parameter_patches_product_yaml(self, tmp_path: Path):
        ...
        try:
            async def _run():
                await mcp.call_tool("specterqa_init", {"directory": str(tmp_path), "url": custom_url})
    ...
    def test_url_parameter_mentioned_in_next_steps(self, tmp_path: Path):
        ...
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_init", {"directory": str(tmp_path), "url": custom_url})
                return _json_result(result)
    ...
    def test_project_dir_in_success_response(self, tmp_path: Path):
        ...
        try:
            ...
            data = run_async(_run())
            assert ".specterqa" in data["project_dir"]
        finally:
            _restore_allowed(srv_module, original)

    def test_error_for_disallowed_directory(self, tmp_path: Path):
        """specterqa_init should return an error if the directory is not in the allowlist."""
        import specterqa.mcp.server as srv_module
        from specterqa.mcp.server import create_server
        ...
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_init", {"directory": "/etc"})
                return _json_result(result)
...
# ---------------------------------------------------------------------------
# 10. specterqa_list_products MCP tool
# ---------------------------------------------------------------------------

class TestSpecterqaListProductsTool:
    """specterqa_list_products tool returns product list from .specterqa/products/."""

    def test_error_for_uninitialized_project(self, tmp_path: Path):
        mcp, srv_module, original = _make_server_with_allowed(tmp_path)
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_list_products", {"directory": str(tmp_path)})
                return _json_result(result)
    ...
    def test_returns_product_from_flat_yaml(self, tmp_path: Path):
        mcp, srv_module, original = _make_server_with_allowed(tmp_path)
        specterqa_dir = _make_initialized_project(tmp_path)
        product_data = {
            "product": {
                "name": "MyApp",
        ...
        (specterqa_dir / "products" / "myapp.yaml").write_text(yaml.dump(product_data), encoding="utf-8")
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_list_products", {"directory": str(tmp_path)})
                return _json_result(result)
    ...
    def test_extracts_url_from_services_frontend(self, tmp_path: Path):
        ...
        (specterqa_dir / "products" / "serviceapp.yaml").write_text(yaml.dump(product_data), encoding="utf-8")
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_list_products", {"directory": str(tmp_path)})
                return _json_result(result)
    ...
    def test_includes_journeys_field(self, tmp_path: Path):
        ...
        (specterqa_dir / "products" / "app.yaml").write_text(yaml.dump(product_data), encoding="utf-8")
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_list_products", {"directory": str(tmp_path)})
                return _json_result(result)
    ...
    def test_includes_journeys_when_present(self, tmp_path: Path):
        ...
        (specterqa_dir / "products" / "journeyapp.yaml").write_text(yaml.dump(product_data), encoding="utf-8")
        journey_data = {"scenario": {"id": "login-flow", "name": "Login Flow", "tags": ["smoke"]}}
        (specterqa_dir / "journeys" / "login.yaml").write_text(yaml.dump(journey_data), encoding="utf-8")
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_list_products", {"directory": str(tmp_path)})
                return _json_result(result)
    ...
    def test_handles_multiple_products(self, tmp_path: Path):
        ...
        for name in ["app-a", "app-b", "app-c"]:
            ...
            (specterqa_dir / "products" / f"{name}.yaml").write_text(yaml.dump(product_data), encoding="utf-8")
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_list_products", {"directory": str(tmp_path)})
                return _json_result(result)
    ...
    def test_defaults_app_type_to_web(self, tmp_path: Path):
        ...
        (specterqa_dir / "products" / "simple.yaml").write_text(yaml.dump(product_data), encoding="utf-8")
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_list_products", {"directory": str(tmp_path)})
                return _json_result(result)
    ...
    def test_finds_directory_style_products(self, tmp_path: Path):
        mcp, srv_module, original = _make_server_with_allowed(tmp_path)
        specterqa_dir = _make_initialized_project(tmp_path)
        product_subdir = specterqa_dir / "products" / "dir-product"
    ...
    def test_skips_invalid_product_yaml_without_crashing(self, tmp_path: Path):
        ...
        (specterqa_dir / "products" / "valid.yaml").write_text(yaml.dump(valid_data), encoding="utf-8")
        (specterqa_dir / "products" / "corrupt.yaml").write_text("{{{{ INVALID", encoding="utf-8")
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_list_products", {"directory": str(tmp_path)})
                return _json_result(result)
    ...
    def test_error_for_disallowed_directory(self, tmp_path: Path):
        ...
        try:
            async def _run():
                result, _ = await mcp.call_tool("specterqa_list_products", {"directory": "/etc"})
                return _json_result(result)
...
# ---------------------------------------------------------------------------
# 11. specterqa_get_results MCP tool
# ---------------------------------------------------------------------------

class TestSpecterqaGetResultsTool:
    """specterqa_get_results tool retrieves stored run results by run ID."""
    ...
    def test_error_when_evidence_dir_missing(self, tmp_path: Path):
        mcp, srv_module, original = _make_server_with_allowed(tmp_path)
        specterqa_dir = tmp_path / ".specterqa"
        specterqa_dir.mkdir()  # No evidence/ subdir
        try:
            async def _run():
                result, _ = await mcp.call_tool(
                    "specterqa_get_results",
    ...
    def test_lists_available_run_ids_in_error(self, tmp_path: Path):
        mcp, srv_module, original = _make_server_with_allowed(tmp_path)
        specterqa_dir = _make_initialized_project(tmp_path)
        evidence_dir = specterqa_dir / "evidence"
...
# ---------------------------------------------------------------------------
# 12. specterqa_run MCP tool — error paths and mocked runs
# ---------------------------------------------------------------------------

class TestSpecterqaRunTool:
    """specterqa_run tool — test error paths and guard checks."""
    ...
    def test_run_with_mocked_orchestrator_returns_passed_true(self, tmp_path: Path):
        ...
        try:
            async def _run():
                with patch("specterqa.mcp.server._build_config") as mock_cfg, \
                     patch("specterqa.mcp.server._check_playwright_available", return_value=None), \
                     patch("specterqa.engine.orchestrator.SpecterQAOrchestrator", return_value=mock_orchestrator):
                    from specterqa.config import SpecterQAConfig
                    cfg = SpecterQAConfig()
                    cfg.project_dir = tmp_path / ".specterqa"
                    cfg.evidence_dir = tmp_path / ".specterqa" / "evidence"
                    cfg.level = "smoke"
        ...
        try:
            async def _run():
                with patch("specterqa.mcp.server._build_config") as mock_cfg, \
                    ...
                    cfg = SpecterQAConfig()
                    cfg.project_dir = specterqa_dir
                    cfg.evidence_dir = specterqa_dir / "evidence"
                    cfg.level = "smoke"
                    mock_cfg.return_value = cfg
